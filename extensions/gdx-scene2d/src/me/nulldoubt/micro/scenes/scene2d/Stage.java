...
import me.nulldoubt.micro.utils.viewport.Viewport;

public class Stage implements InputProcessor, Disposable {
	...
	public Stage(Viewport viewport, Batch batch) {
		...
		viewport.update(Micro.graphics.getWidth(), Micro.graphics.getHeight(), true);
	}
	...
	private void drawDebug() {
		...
		if (debugUnderMouse || debugParentUnderMouse || debugTableUnderMouse != Debug.none) {
			screenToStageCoordinates(tempCoords.set(Micro.input.getX(), Micro.input.getY()));
	 ...
	 */
	public void act() {
		act(Math.min(Micro.graphics.getDeltaTime(), 1 / 30f));
	}
	...
	public void act(float delta) {
		...
		ApplicationType type = Micro.app.getType();
		if (type == ApplicationType.Desktop || type == ApplicationType.Applet)
	...
	public Vector2 stageToScreenCoordinates(Vector2 stageCoords) {
		...
		stageCoords.y = Micro.graphics.getHeight() - stageCoords.y;
		return stageCoords;
	}
