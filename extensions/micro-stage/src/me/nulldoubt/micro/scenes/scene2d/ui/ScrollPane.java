	...
	protected ActorGestureListener getFlickScrollListener() {
		return new ActorGestureListener() {
			public void pan(InputEvent event, float x, float y, float deltaX, float deltaY) {
				...
				amountY += deltaY;
				clamp();
				if (cancelTouchFocus && (deltaX != 0 || deltaY != 0))
					cancelTouchFocus();
			}
	...
	public void layout() {
		...
		if (!scrollbarsOnTop) {
			if (scrollY) {
				...
				if (!vScrollOnRight)
					actorArea.x += scrollbarWidth;
				// Horizontal scrollbar may cause vertical scrollbar to show.
				if (!scrollX && actorWidth > actorArea.width && !disableX)
					scrollX = true;
			}
			if (scrollX) {
				actorArea.height -= scrollbarHeight;
				if (hScrollOnBottom)
					actorArea.y += scrollbarHeight;
		...
		if (scrollX) {
			if (hScrollKnob != null) {
				...
				if (hKnobBounds.width > actorWidth)
					hKnobBounds.width = 0;
		...
		if (scrollY) {
			if (vScrollKnob != null) {
				...
				if (vKnobBounds.height > actorHeight)
