...
static inline set_pixel_func set_pixel_func_ptr(uint32_t format) {
	switch(format) {
		case MICRO2D_FORMAT_ALPHA:			return &set_pixel_alpha;
		case MICRO2D_FORMAT_LUMINANCE_ALPHA:	return &set_pixel_luminance_alpha;
		case MICRO2D_FORMAT_RGB888:			return &set_pixel_RGB888;
		case MICRO2D_FORMAT_RGBA8888:			return &set_pixel_RGBA8888;
		case MICRO2D_FORMAT_RGB565:			return &set_pixel_RGB565;
		case MICRO2D_FORMAT_RGBA4444:			return &set_pixel_RGBA4444;
...
static inline get_pixel_func get_pixel_func_ptr(uint32_t format) {
	switch(format) {
		case MICRO2D_FORMAT_ALPHA:			return &get_pixel_alpha;
		case MICRO2D_FORMAT_LUMINANCE_ALPHA:	return &get_pixel_luminance_alpha;
		case MICRO2D_FORMAT_RGB888:			return &get_pixel_RGB888;
		case MICRO2D_FORMAT_RGBA8888:			return &get_pixel_RGBA8888;
		case MICRO2D_FORMAT_RGB565:			return &get_pixel_RGB565;
		case MICRO2D_FORMAT_RGBA4444:			return &get_pixel_RGBA4444;
		default: return &get_pixel_alpha; // better idea for a default?
	}
}

micro2d_pixmap* micro2d_load(const unsigned char *buffer, uint32_t len) {
	...
	micro2d_pixmap* pixmap = (micro2d_pixmap*)malloc(sizeof(micro2d_pixmap));
	if (!pixmap) return 0;
	pixmap->width = (uint32_t)width;
	pixmap->height = (uint32_t)height;
	pixmap->format = (uint32_t)format;
	pixmap->blend = MICRO2D_BLEND_SRC_OVER;
	pixmap->scale = MICRO2D_SCALE_BILINEAR;
	pixmap->pixels = pixels;
	return pixmap;
}

uint32_t micro2d_bytes_per_pixel(uint32_t format) {
	switch(format) {
		case MICRO2D_FORMAT_ALPHA:
			return 1;
		case MICRO2D_FORMAT_LUMINANCE_ALPHA:
...
micro2d_pixmap* micro2d_new(uint32_t width, uint32_t height, uint32_t format) {
	micro2d_pixmap* pixmap = (micro2d_pixmap*)malloc(sizeof(micro2d_pixmap));
	if (!pixmap) return 0;
	pixmap->width = width;
	...
	pixmap->blend = MICRO2D_BLEND_SRC_OVER;
	pixmap->scale = MICRO2D_SCALE_BILINEAR;
	pixmap->pixels = (unsigned char*)malloc(width * height * micro2d_bytes_per_pixel(format));
	if (!pixmap->pixels) {
		free((void*)pixmap);
		return 0;
	}
...
void micro2d_free(const micro2d_pixmap* pixmap) {
	free((void*)pixmap->pixels);
	free((void*)pixmap);
}

void micro2d_set_blend (micro2d_pixmap* pixmap, uint32_t blend) {
    pixmap->blend = blend;
}

void micro2d_set_scale (micro2d_pixmap* pixmap, uint32_t scale) {
	pixmap->scale = scale;
}

const char *micro2d_get_failure_reason(void) {
	return stbi_failure_reason();
}

static inline void clear_alpha(const micro2d_pixmap* pixmap, uint32_t col) {
	int pixels = pixmap->width * pixmap->height;
	memset((void*)pixmap->pixels, col, pixels);
}

static inline void clear_luminance_alpha(const micro2d_pixmap* pixmap, uint32_t col) {
	int pixels = pixmap->width * pixmap->height;
...
static inline void clear_RGB888(const micro2d_pixmap* pixmap, uint32_t col) {
	int pixels = pixmap->width * pixmap->height;
...
static inline void clear_RGBA8888(const micro2d_pixmap* pixmap, uint32_t col) {
	int pixels = pixmap->width * pixmap->height;
...
static inline void clear_RGB565(const micro2d_pixmap* pixmap, uint32_t col) {
	int pixels = pixmap->width * pixmap->height;
	unsigned short* ptr = (unsigned short*)pixmap->pixels;
...
static inline void clear_RGBA4444(const micro2d_pixmap* pixmap, uint32_t col) {
	int pixels = pixmap->width * pixmap->height;
	unsigned short* ptr = (unsigned short*)pixmap->pixels;
	unsigned short l = col & 0xffff;
...
void micro2d_clear(const micro2d_pixmap* pixmap, uint32_t col) {
...
static inline int32_t in_pixmap(const micro2d_pixmap* pixmap, int32_t x, int32_t y) {
	if(x < 0 || y < 0)
		return 0;
...
uint32_t micro2d_get_pixel(const micro2d_pixmap* pixmap, int32_t x, int32_t y) {
	if(!in_pixmap(pixmap, x, y))
		return 0;
	unsigned char* ptr = (unsigned char*)pixmap->pixels + (x + pixmap->width * y) * micro2d_bytes_per_pixel(pixmap->format);
	return to_RGBA8888(pixmap->format, get_pixel_func_ptr(pixmap->format)(ptr));
}

void micro2d_set_pixel(const micro2d_pixmap* pixmap, int32_t x, int32_t y, uint32_t col) {
	if(pixmap->blend) {
		uint32_t dst = micro2d_get_pixel(pixmap, x, y);
		col = blend(col, dst);
		col = to_format(pixmap->format, col);
		set_pixel((unsigned char*)pixmap->pixels, pixmap->width, pixmap->height, micro2d_bytes_per_pixel(pixmap->format), set_pixel_func_ptr(pixmap->format), x, y, col);
	} else {
		col = to_format(pixmap->format, col);
		set_pixel((unsigned char*)pixmap->pixels, pixmap->width, pixmap->height, micro2d_bytes_per_pixel(pixmap->format), set_pixel_func_ptr(pixmap->format), x, y, col);
	}
...
void micro2d_draw_line(const micro2d_pixmap* pixmap, int32_t x0, int32_t y0, int32_t x1, int32_t y1, uint32_t col) {
    int32_t dy = y1 - y0;
    int32_t dx = x1 - x0;
	int32_t fraction = 0;
    int32_t stepx, stepy;
	unsigned char* ptr = (unsigned char*)pixmap->pixels;
	uint32_t bpp = micro2d_bytes_per_pixel(pixmap->format);
...
static inline void hline(const micro2d_pixmap* pixmap, int32_t x1, int32_t x2, int32_t y, uint32_t col) {
	int32_t tmp = 0;
	set_pixel_func pset = set_pixel_func_ptr(pixmap->format);
	get_pixel_func pget = get_pixel_func_ptr(pixmap->format);
	unsigned char* ptr = (unsigned char*)pixmap->pixels;
...
static inline void vline(const micro2d_pixmap* pixmap, int32_t y1, int32_t y2, int32_t x, uint32_t col) {
	int32_t tmp = 0;
	set_pixel_func pset = set_pixel_func_ptr(pixmap->format);
	get_pixel_func pget = get_pixel_func_ptr(pixmap->format);
	unsigned char* ptr = (unsigned char*)pixmap->pixels;
...
void micro2d_draw_rect(const micro2d_pixmap* pixmap, int32_t x, int32_t y, uint32_t width, uint32_t height, uint32_t col) {
...
void micro2d_draw_circle(const micro2d_pixmap* pixmap, int32_t x, int32_t y, uint32_t radius, uint32_t col) {
    int32_t px = 0;
    int32_t py = radius;
...
void micro2d_fill_rect(const micro2d_pixmap* pixmap, int32_t x, int32_t y, uint32_t width, uint32_t height, uint32_t col) {
...
void micro2d_fill_circle(const micro2d_pixmap* pixmap, int32_t x0, int32_t y0, uint32_t radius, uint32_t col) {
...
void micro2d_fill_triangle(const micro2d_pixmap* pixmap, int32_t x1, int32_t y1, int32_t x2, int32_t y2, int32_t x3, int32_t y3, uint32_t col) {
...
static inline void blit_same_size(const micro2d_pixmap* src_pixmap, const micro2d_pixmap* dst_pixmap,
	...
	uint32_t sbpp = micro2d_bytes_per_pixel(src_pixmap->format);
	uint32_t dbpp = micro2d_bytes_per_pixel(dst_pixmap->format);
...
static inline void blit_bilinear(const micro2d_pixmap* src_pixmap, const micro2d_pixmap* dst_pixmap,
...
static inline void blit_linear(const micro2d_pixmap* src_pixmap, const micro2d_pixmap* dst_pixmap,
		   int32_t src_x, int32_t src_y, uint32_t src_width, uint32_t src_height,
		   int32_t dst_x, int32_t dst_y, uint32_t dst_width, uint32_t dst_height) {
	set_pixel_func pset = set_pixel_func_ptr(dst_pixmap->format);
	get_pixel_func pget = get_pixel_func_ptr(src_pixmap->format);
...
static inline void blit(const micro2d_pixmap* src_pixmap, const micro2d_pixmap* dst_pixmap,
					   int32_t src_x, int32_t src_y, uint32_t src_width, uint32_t src_height,
					   int32_t dst_x, int32_t dst_y, uint32_t dst_width, uint32_t dst_height) {
	if(dst_pixmap->scale == MICRO2D_SCALE_NEAREST)
		blit_linear(src_pixmap, dst_pixmap, src_x, src_y, src_width, src_height, dst_x, dst_y, dst_width, dst_height);
	if(dst_pixmap->scale == MICRO2D_SCALE_BILINEAR)
		blit_bilinear(src_pixmap, dst_pixmap, src_x, src_y, src_width, src_height, dst_x, dst_y, dst_width, dst_height);
}

void micro2d_draw_pixmap(const micro2d_pixmap* src_pixmap, const micro2d_pixmap* dst_pixmap,
					   int32_t src_x, int32_t src_y, uint32_t src_width, uint32_t src_height,
					   int32_t dst_x, int32_t dst_y, uint32_t dst_width, uint32_t dst_height) {
