...
class Lwjgl3GL20 implements me.nulldoubt.micro.graphics.GL20 {
	...
	public void glDrawElements(int mode, int count, int type, Buffer indices) {
		if (indices instanceof ShortBuffer sb && type == me.nulldoubt.micro.graphics.GL20.GL_UNSIGNED_SHORT) {
			int position = sb.position();
			int oldLimit = sb.limit();
			sb.limit(position + count);
			GL11.glDrawElements(mode, sb);
			sb.limit(oldLimit);
		} else if (indices instanceof ByteBuffer && type == me.nulldoubt.micro.graphics.GL20.GL_UNSIGNED_SHORT) {
			...
			GL11.glDrawElements(mode, sb);
			sb.limit(oldLimit);
		} else if (indices instanceof ByteBuffer bb && type == me.nulldoubt.micro.graphics.GL20.GL_UNSIGNED_BYTE) {
	...
	public void glBufferSubData(int target, int offset, int size, Buffer data) {
		if (data == null)
			throw new MicroRuntimeException("Using null for the data not possible, blame LWJGL");
		else if (data instanceof ByteBuffer)
