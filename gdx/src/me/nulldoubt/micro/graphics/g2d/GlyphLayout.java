...
public class GlyphLayout implements Poolable {
	...
	private static final Pool<GlyphRun> glyphRunPool = Pools.get(GlyphRun.class, GlyphRun::new);
	...
	private void setLastGlyphXAdvance(BitmapFontData fontData, GlyphRun run) {
		...
		if (!last.fixedWidth)
			run.xAdvances.items[run.xAdvances.size - 1] = getGlyphWidth(last, fontData);
	}
