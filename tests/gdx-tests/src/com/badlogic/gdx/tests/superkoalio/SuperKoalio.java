...
public class SuperKoalio extends GdxTest {
	...
	public void render () {
		...
		// get the delta time
		float deltaTime = Micro.graphics.getDeltaTime();
	...
	private void updateKoala (float deltaTime) {
		...
		if ((Micro.input.isKeyPressed(Keys.SPACE) || isTouched(0.5f, 1)) && koala.grounded) {
			koala.velocity.y += Koala.JUMP_VELOCITY;
			koala.state = Koala.State.Jumping;
			koala.grounded = false;
		}

		if (Micro.input.isKeyPressed(Keys.LEFT) || Micro.input.isKeyPressed(Keys.A) || isTouched(0, 0.25f)) {
			koala.velocity.x = -Koala.MAX_VELOCITY;
			if (koala.grounded) koala.state = Koala.State.Walking;
			koala.facesRight = false;
		}

		if (Micro.input.isKeyPressed(Keys.RIGHT) || Micro.input.isKeyPressed(Keys.D) || isTouched(0.25f, 0.5f)) {
			koala.velocity.x = Koala.MAX_VELOCITY;
			if (koala.grounded) koala.state = Koala.State.Walking;
			koala.facesRight = true;
		}

		if (Micro.input.isKeyJustPressed(Keys.B)) debug = !debug;
	...
	private boolean isTouched (float startX, float endX) {
		...
		for (int i = 0; i < 2; i++) {
			float x = Micro.input.getX(i) / (float) Micro.graphics.getWidth();
			if (Micro.input.isTouched(i) && (x >= startX && x <= endX)) {
				return true;
			}
		}
		return false;
