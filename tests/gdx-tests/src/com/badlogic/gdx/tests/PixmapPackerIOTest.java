...
public class PixmapPackerIOTest extends GdxTest {
	...
	public void create () {
		...
		try {
			...
			pixmapPackerIO.save(Micro.files.local("pixmapPackerTest.atlas"), packer, saveParameters);
		} catch (IOException e) {
			e.printStackTrace();
		}

		TextureAtlas loaded = new TextureAtlas(Micro.files.local("pixmapPackerTest.atlas"));
