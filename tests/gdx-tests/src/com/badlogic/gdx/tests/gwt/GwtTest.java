...
public class GwtTest extends GdxTest {
	...
	@Override
	public void create () {
		Preferences pref = Micro.app.getPreferences("test");
		...
		texture = new Texture(new Pixmap(Micro.files.internal("data/badlogic.jpg")), true);
		texture.setFilter(TextureFilter.MipMap, TextureFilter.Linear);

		String params = Micro.files.internal("data/gwttestparams.txt").readString();
		numSprites = Integer.parseInt(params);

		batch = new SpriteBatch();
		positions = new ArrayList<Vector2>();
		for (int i = 0; i < numSprites; i++) {
			positions.add(new Vector2(MathUtils.random() * Micro.graphics.getWidth(), MathUtils.random() * Micro.graphics.getHeight()));
		}
		...
		atlas = new TextureAtlas(Micro.files.internal("data/pack.atlas"));
	}
	...
	public void render () {
		...
		sprite.rotate(Micro.graphics.getDeltaTime() * 45);
		for (Vector2 position : positions) {
			sprite.setPosition(position.x, position.y);
			sprite.draw(batch);
		}
		font.draw(batch,
			"fps:" + Micro.graphics.getFPS() + ", delta: " + Micro.graphics.getDeltaTime() + ", #sprites: " + numSprites, 0,
			30);
