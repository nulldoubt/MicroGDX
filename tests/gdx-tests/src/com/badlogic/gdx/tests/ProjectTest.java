...
public class ProjectTest extends GdxTest {
	...
	public void create () {
		...
		sphere = objLoader.loadModel(Micro.files.internal("data/sphere.obj"));
		sphere.materials.get(0).set(new ColorAttribute(ColorAttribute.Diffuse, Color.WHITE));
		cam = new PerspectiveCamera(45, Micro.graphics.getWidth(), Micro.graphics.getHeight());
		cam.far = 200;
		...
		font = new BitmapFont();
		logo = new TextureRegion(new Texture(Micro.files.internal("data/badlogicsmall.jpg")));
	...
	public void render () {
		...
		if (Micro.input.isKeyPressed(Keys.A)) cam.rotate(20 * Micro.graphics.getDeltaTime(), 0, 1, 0);
		if (Micro.input.isKeyPressed(Keys.D)) cam.rotate(-20 * Micro.graphics.getDeltaTime(), 0, 1, 0);
