...
public class InterpolationTest extends GdxTest {
	...
	@Override
	public void create () {
		Micro.gl.glClearColor(.3f, .3f, .3f, 1);
		...
		Micro.input.setInputProcessor(new InputMultiplexer(new InputAdapter() {
			public boolean scrolled (float amountX, float amountY) {
				if (!Micro.input.isKeyPressed(Keys.CONTROL_LEFT)) return false;
	...
	public void render () {
		...
		float bottomLeftX = Micro.graphics.getWidth() / 2 - graphSize / 2, bottomLeftY = Micro.graphics.getHeight() / 2 - graphSize / 2;
