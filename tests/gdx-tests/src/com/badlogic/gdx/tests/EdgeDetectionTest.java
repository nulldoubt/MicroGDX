...
public class EdgeDetectionTest extends GdxTest {
	...
	public void create () {
		...
		scene = objLoader.loadModel(Micro.files.internal("data/scene.obj"));
		sceneInstance = new ModelInstance(scene);
		modelBatch = new ModelBatch();
		fbo = new FrameBuffer(Format.RGB565, Micro.graphics.getWidth(), Micro.graphics.getHeight(), true);
	...
	private void calculateOffsets () {
		...
		for (int y = -1; y <= 1; y++) {
			for (int x = -1; x <= 1; x++) {
				offsets[idx++] = x / (float) Micro.graphics.getWidth();
				offsets[idx++] = y / (float) Micro.graphics.getHeight();
			}
		}
	...
	public void render () {
		angle += 45 * Micro.graphics.getDeltaTime();
		...
		batch.draw(fboRegion, 0, 0, Micro.graphics.getWidth(), Micro.graphics.getHeight());
		batch.end();
		logger.log();
	}
}
