...
public class ParticleEmittersTest extends GdxTest {
	...
	public void create () {
		...
		effect.load(Micro.files.internal("data/singleTextureAllAdditive.p"), Micro.files.internal("data"));
		...
		InputProcessor inputProcessor = new InputAdapter() {
			...
			public boolean touchDragged (int x, int y, int pointer) {
				if (latestEffect != null) latestEffect.setPosition(x, Micro.graphics.getHeight() - y);
				return false;
			}
			...
			public boolean touchDown (int x, int y, int pointer, int newParam) {
				...
				latestEffect.setPosition(x, Micro.graphics.getHeight() - y);
		...
		multiplexer.add(ui);
		multiplexer.add(inputProcessor);
	...
	public void render () {
		...
		if (fpsCounter > 3) {
			fpsCounter = 0;
			String log = effects.size + " particle effects, FPS: " + Micro.graphics.getFPS() + ", Render calls: "
				+ spriteBatch.renderCalls;
			Micro.app.log("libGDX", log);
