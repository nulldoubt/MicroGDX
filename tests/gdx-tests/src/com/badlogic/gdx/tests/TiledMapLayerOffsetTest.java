...
public class TiledMapLayerOffsetTest extends GdxTest {
	...
	@Override
	public void render () {
		if (Micro.input.isKeyJustPressed(Input.Keys.NUM_1)) {
			if (mapType != 0) {
		...
		} else if (Micro.input.isKeyJustPressed(Input.Keys.NUM_2)) {
			if (mapType != 1) {
				if (renderer instanceof Disposable) ((Disposable)renderer).dispose();
				mapType = 1;
				map = assetManager.get(MAP_ORTHO);
		...
		} else if (Micro.input.isKeyJustPressed(Input.Keys.NUM_3)) {
			if (mapType != 2) {
				if (renderer instanceof Disposable) ((Disposable)renderer).dispose();
				mapType = 2;
		...
		} else if (Micro.input.isKeyJustPressed(Input.Keys.NUM_4)) {
			if (mapType != 3) {
				if (renderer instanceof Disposable) ((Disposable)renderer).dispose();
				mapType = 3;
		...
		} else if (Micro.input.isKeyJustPressed(Input.Keys.NUM_5)) {
			if (mapType != 4) {
				if (renderer instanceof Disposable) ((Disposable)renderer).dispose();
				mapType = 4;
		...
		} else if (Micro.input.isKeyJustPressed(Input.Keys.NUM_6)) {
			if (mapType != 5) {
				if (renderer instanceof Disposable) ((Disposable)renderer).dispose();
				mapType = 5;
		...
		font.draw(batch, "Switch type with 1-6", Micro.graphics.getHeight() - 100, 50);
		font.draw(batch, renderer.getClass().getSimpleName(), Micro.graphics.getHeight() - 100, 20);
		batch.end();
	}
}
