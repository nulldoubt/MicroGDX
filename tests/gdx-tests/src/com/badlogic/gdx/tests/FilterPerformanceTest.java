...
public class FilterPerformanceTest extends GdxTest {
	...
	void setTextureFilter (int filter) {
		...
		Micro.gl.glTexParameterf(GL20.GL_TEXTURE_2D, GL20.GL_TEXTURE_MIN_FILTER, filters[filter]);
		texture.bind();
		Micro.gl.glTexParameterf(GL20.GL_TEXTURE_2D, GL20.GL_TEXTURE_MIN_FILTER, filters[filter]);
	}
	...
	public void create () {
		...
		texture = new Texture(Micro.files.internal("data/resource1.jpg"), true);
	...
	public void render () {
		...
		batch.begin();
		font.draw(batch, modeString + " fps:" + Micro.graphics.getFPS(), 26, 65);
		batch.end();
	}
