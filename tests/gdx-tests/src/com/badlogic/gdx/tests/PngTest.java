...
public class PngTest extends GdxTest {
	...
	public void create () {
		...
		badlogic = new Texture(Micro.files.internal("data/badlogic.jpg"));
	}
	...
	public void render () {
		...
		batch.begin();
		if (screenshot == null) {
			int width = Micro.graphics.getWidth(), height = Micro.graphics.getHeight();
			for (int i = 0; i < 100; i++)
			...
			Pixmap pixmap = Pixmap.createFromFrameBuffer(0, 0, Micro.graphics.getWidth(), Micro.graphics.getHeight());
			try {
